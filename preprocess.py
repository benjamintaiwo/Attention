...
def MakeS2SDict(fn=None, min_freq=5, delimiter=' ', dict_file=None):
	...
	wlists = []
	for wd in wdicts:	
		wd = data_handler.FreqDict2List(wd)
	...
	if dict_file is not None:
		data_handler.SaveList(wlists[0]+['<@@@>']+wlists[1], dict_file)
	return itokens, otokens
...
def S2SDataGenerator(fn, itokens, otokens, batch_size=64, delimiter=' ', max_len=999):
	Xs = [[], []]
	while True:
		for ss in data_handler.LoadCSVg(fn):
			for seq, xs in zip(ss, Xs):
				xs.append(list(seq.split(delimiter)))
...
if __name__ == '__main__':
	itokens, otokens = MakeS2SDict('train.txt', 6, dict_file='train_word.txt')
	X, Y = MakeS2SData('train.txt', itokens, otokens, h5_file='train.h5')
